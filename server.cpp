...
/*void process_newchannel_request (TCPRequestChannel *_channel){
	...
	TCPRequestChannel *data_channel = new FIFORequestChannel (new_channel_name, FIFORequestChannel::SERVER_SIDE);
...
int main(int argc, char *argv[]){
	...
	while(true){
		...
		int clientSocket = accept(control_channel ->getfd(), (struct sockaddr*)&clientAddress, &size);
		if(clientSocket == -1){
			continue;
		}
		TCPRequestChannel* client_channel = new TCPRequestChannel(clientSocket);
